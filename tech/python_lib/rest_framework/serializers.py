...
class BaseSerializer(Field):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        # 如果传递了many=True参数那么序列化器会自动使用ListSerializer对象
    ...
    @property
    def data(self):
        # 要么传递了初始化时传递了data 要么就必须经过了is_valid()进行了验证,因为只有进行了is_valid验证之后,才会拥有_validated_data属性
        ...
        if not hasattr(self, '_data'):
            # 如果 有 instance对象,并且没有_errors属性
            if self.instance is not None and not getattr(self, '_errors', None):
                # 将python内置数据转化为序列化之前的数据
                self._data = self.to_representation(self.instance)
            # 如果有_validated_data属性,并且没有_errors属性
            # 就把验证后的数据进行输出
    ...
    def validated_data(self):
        # 必须经过is_valid验证才会有_validated_data属性
        if not hasattr(self, '_validated_data'):
...
class SerializerMetaclass(type):
    ...
    def _get_declared_fields(cls, bases, attrs):
        # 判断哪些是Field对象,然后全部加入到_declared_fields字典属性中
...
class Serializer(BaseSerializer):
    ...
    def run_validation(self, data=empty):
        ...
        if is_empty_value:
            return data
        # 对每一个字段进行校验,会先调用字段的validators中校验函数,
        # 再调用序列化器中的validate_{field_name}方法
        # 返回的是有序字典数据
    ...
    def to_internal_value(self, data):
        ...
        # 对所有字段进行数据校验
        for field in fields:
            # 找出validate_{field_name}验证函数
            validate_method = getattr(self, 'validate_' + field.field_name, None)
            # 通过primitive_value=data.get(field_name) 拿到字段对应的原始值
            primitive_value = field.get_value(data)
            try:
                validated_value = field.run_validation(primitive_value)
                # 校验字段的值
                # 先转化为python内置对象
                # value = self.to_internal_value(data) # 这里也可以抛出异常
                # 循环调用字段的validators中的校验函数进行校验,这里的校验函数不需要返回值,只需要在校验失败时,抛出ValidationError就行
                # self.run_validators(value)
                # return value

                if validate_method is not None:
                    validated_value = validate_method(validated_value)
                    # 调用序列化器类中的validate_{field_name}方法校验
                    # 这里要注意返回值是校验函数的返回值
                    # 所以校验函数一定要返回值
            except ValidationError as exc:
    ...
    def to_representation(self, instance):
        ...
        for field in fields:
            ...
            if check_for_none is None:
                ...
                # 调用字段的to_representation方法,把值转化为需要输出的格式
    ...
    @property
    def data(self):
        # 调用基类的data,实际就是调用self.to_representation
