...
class Field(object):
    ...
    def __init__(self, read_only=False, write_only=False,
        ...
        # 这里会把子类中定义的 default_error_messages 以及构造参数传递的error_messages 字典合并为最终的错误字典
        messages = {}
    ...
    def fail(self, key, **kwargs):
        ...
        对于一个字段来说,如果验证发生了错误,错误的详细信息使用通过error_messages选项来声明的
        """
        try:
            # 当字段验证出错的时候,会拿到通过验证错误的key
            # 比如说max_length错误,那么是通过 error_messages['max_length']来拿到错误详细内容
            msg = self.error_messages[key]
        except KeyError:
            # 如果error_messages中没有这个错误key,那么抛出 当前错误详细内容不存在于error_messages
...
class BooleanField(Field):
    ...
    def __init__(self, **kwargs):
        # 不支持allow_null选项,这里只允许存储True和False
...
class CharField(Field):
    ...
    def to_internal_value(self, data):
        ...
        # 如果传递进来的是True或者False 获取不是字符串类型就报invalid
...
class RegexField(CharField):
    ...
    def __init__(self, regex, **kwargs):
        ...
        # 添加正则验证 通过给定的正则表达式来检查 给定的字符串中匹配 regex.search(data)
...
class SlugField(CharField):
    ...
    def __init__(self, allow_unicode=False, **kwargs):
        ...
        # 给定了正则匹配符re.compile(r'^[-\w]+\Z', re.UNICODE)
...
class IPAddressField(CharField):
    ...
    # "192.168.10.1" "192.168.1.1.1"
    def __init__(self, protocol='both', **kwargs):
        # 默认验证ipv4
        # protocol=both 验证 ipv4,6
...
class IntegerField(Field):
    ...
    def __init__(self, **kwargs):
        ...
        # 添加最大值和最小值判断 类似于字符串类型的最大值和最小值判断
    ...
    def to_internal_value(self, data):
        # 可以传递字符串进来,但是必须要合法的字符串
        if isinstance(data, six.text_type) and len(data) > self.MAX_STRING_LENGTH:
            self.fail('max_string_length')

        try:
            # 先把data转化为字符串,在通过正则进行把r'\.0*\s*$'字符串去掉 '123123.0    '会变成'123123'
...
class ChoiceField(Field):
    ...
    def _set_choices(self, choices):
        ...
        # (1,'male')
        # )
        # 那么就是{0:'female',1:"male"}
        self.grouped_choices = to_choices_dict(choices)
        # 和grouped_choices 一样 {0:'female',1:"male"}
...
class MultipleChoiceField(ChoiceField):
    # 继承自ChoiceField choices指明可以选择的范围,并且可以选择多个
    default_error_messages = {
        'invalid_choice': _('"{input}" is not a valid choice.'),
...
class FileField(Field):
    ...
    def to_internal_value(self, data):
        ...
        if not file_name:
            self.fail('no_name')
        # 如果文件没有大小file_size,并且不允许为空的话那么就报错
    ...
    def to_internal_value(self, data):
        ...
        try:
            if self.binary or getattr(data, 'is_json_string', False):
                # 如果是二进制数据就先decode,然后loads出来
                if isinstance(data, six.binary_type):
                    data = data.decode('utf-8')
                
                return json.loads(data)
            else:
                # 如果self.binary为False或者data没有is_json_string方法,那么尝试使用json.dumps是否报错,不报错说明已经是python内部字典或者数组了,直接在最后return data
                json.dumps(data)
        except (TypeError, ValueError):
            self.fail('invalid')
        return data
...
# 隐藏字段,不会从用户接受数据,他使用default来指定值
# 并且这个字段的值会添加到validated_data中
...
# 字段 example = SerializerMethodField() 只用于序列化输出,他会使用函数返回值作为值
# 函数名 get_{example}(self,obj):# obj为instance参数值
class SerializerMethodField(Field):
    ...
    def __init__(self, method_name=None, **kwargs):
        # 可以指明调用哪一个方法
        # 默认是get_{field_name}方法
        self.method_name = method_name
        kwargs['source'] = '*'
        kwargs['read_only'] = True
        super(SerializerMethodField, self).__init__(**kwargs)

    # 绑定parent也就是字段所在的序列化对象
    def bind(self, field_name, parent):
        # In order to enforce a consistent style, we error if a redundant
...
# 专门给ModelSerializer使用的,用于直接映射模型的字段和序列化器的字段
class ModelField(Field):
    ...
    def to_internal_value(self, data):
        # 模型类关联的_meta里面保存了字段,找到字段字段,通过to_python来把值得转化交给模型类字段来处理,
        # 这里就是拿到模型字段的值
